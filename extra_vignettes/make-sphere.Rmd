...
Use the `sphere` function to reproduce the examples below.

![](figure/minecraft-spheres.png)

## 0. Connect to Minecraft server
...
## 2. Create a stack of rings

This step creates a single hemisphere where the `y` dimension represents height in the minecraft world. In a later step we will duplicate the hemisphere to create a full sphere. See [Wolfram](http://mathworld.wolfram.com/Sphere.html) for the formulae used to create the rings.
...
## 6. Place blocks to create the sphere
...
Consider these examples to create a sphere, dome, floating island, a Death Star, and more.
...
# Gobstopper
for(i in 1:5 * 5){
  sphere(i, blockid = 95, styleid = i / 5 * 2, offset = c(0, 40, 0))
}
...
# Mouse ears
sphere(16, 49, 0, TRUE, zlim = c(0, 0), offset = c(  0,  0, 5))
sphere( 8, 49, 0, TRUE, zlim = c(0, 0), offset = c( 16, 17, 5))
sphere( 8, 49, 0, TRUE, zlim = c(0, 0), offset = c(-16, 17, 5))
```
